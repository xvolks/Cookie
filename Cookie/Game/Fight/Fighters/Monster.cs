...
namespace Cookie.Game.Fight.Fighters
{
    ...
    {
        public Monster(int id, int cellId, GameFightMinimalStats stats, uint teamId, bool isAlive, ushort creatureGenericId, byte creatureGrade) : base(id, cellId, stats, teamId, isAlive)
        {
            CreatureGenericId = creatureGenericId;
            CreatureGrade = creatureGrade;
            Name = (string)API.Gamedata.D2OParsing.GetMonsterName(creatureGenericId);
        }
