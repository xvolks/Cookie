            ...
            foreach (var actor in actors)
                if (actor is GameRolePlayGroupMonsterInformations monster)
                    Monsters.Add(new MonsterGroup(monster.StaticInfos,
                        monster.StaticInfos.MainCreatureLightInfos.CreatureGenericId, monster.Disposition.CellId));
                else if (actor is GameRolePlayNpcInformations npc)
                    Npcs.Add(new Npc(npc.Disposition.CellId, (int) npc.ContextualId, npc.NpcId));
