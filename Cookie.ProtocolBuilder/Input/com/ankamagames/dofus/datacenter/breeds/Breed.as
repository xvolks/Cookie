package com.ankamagames.dofus.datacenter.breeds
{
   ...
   public class Breed implements IDataCenter
   {
      ...
      public function get breedSpellsWithVariants() : Vector.<Spell>
      {
         var _loc1_:uint = 0;
         var _loc2_:Spell = null;
         var _loc3_:int = 0;
         if(!this._breedSpellsVariants && Spell.getSpellById(1) != null)
         {
            this._breedSpellsVariants = new Vector.<Spell>();
            for each(_loc1_ in this.breedSpellsId)
            {
               _loc2_ = Spell.getSpellById(_loc1_);
               for each(_loc3_ in _loc2_.variants)
               {
                  this._breedSpellsVariants.push(Spell.getSpellById(_loc3_));
               }
            }
         }
         return this._breedSpellsVariants;
      }
